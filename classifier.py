...
def PCA_labelled(fpi_path, labels_dict, n_components=2, pic_filename=''):
    ...
    Example:
        ...
        pca_dict = PCA_labelled(fpi_data_path, lbl_dict, pic_filename=os.path.join(base_path, r'pca_' + year + month + '.pic'))

    """
    labels_dict = mu.read_labels_cdf(os.path.join(lbl_path, r'labels_fpi_fast_dis_dist_' + year + month + '.cdf'))
    ...
    projection = pca.fit_transform(pca_data)
...
def plot_labels_orbit(fgm_dict, lbl_dict):
    ...
    X = interp1d(epoch_state, r[:,0], axis=0, fill_value="extrapolate")(epoch_lbl)
    Y = interp1d(epoch_state, r[:,1], axis=0, fill_value="extrapolate")(epoch_lbl)
